...
- "inoculation_first" -- technique-naming prebunk (Roozenbeek & van der Linden 2022 for primary empirical evidence; McGuire 1964 and van der Linden 2020 for theoretical framing). Appropriate when the reader is still inside the manipulation frame and a direct correction would trigger identity defence. Lead with naming the technique ("this is how X-style argument works"), then introduce counter-evidence WITHIN THE SAME RESPONSE. Use this when the upstream Classifier has flagged Gorgon Trap GT-family TTPs or high manipulation-vector density.
- "relational_first" -- Common Humanity / acknowledgment-first (Perry et al. Common Humanity scale; Costello, Pennycook, Rand 2024 Round 1 acknowledgment move). Appropriate when identity stakes dominate the claim and any correction will be read as attack unless the kernel of truth is acknowledged first. Use this when perception_gap is high, moral_foundations diverge sharply, or the upstream pipeline signals identity-targeting dynamics.
Do not use this field to express uncertainty about the facts. It is advisory to downstream human communicators about the FORM of the response, not the CONTENT.
POSTURE SCOPE (load-bearing, Sprint 2 PR 3 fleet convergence): the posture instructs a downstream communicator about how to present this analysis to an end reader. It does NOT alter the structure, order, or completeness of the analytical layers above. Produce narrative_deconstruction, consensus_explanation, inferential_gap, feasibility_check, and commercial_motives in FULL regardless of posture. No posture ever licenses abbreviating, deferring, or omitting the Inferential Gap Map or any other analytical content.
...
Use this signal ESPECIALLY when the upstream Decomposer has flagged `hypothesis_crowding=high` or `complexity_explosion_flag=true`, as those upstream signals correlate with pattern-dense construction (BG-043 gated-detection-composition discipline). Conversely, if the claim's pattern density is fully explained by a conventional rhetorical form -- protest chants, religious liturgy, legal cumulative argument, poetry, mnemonic structures in educational content -- set this to false. The flag targets engineered mnemonic bonding, not every rhetorical form that uses repetition.

H) Vacuum Filled By (narrative pattern only -- NOT named publishers):
If an expertise or information vacuum around the claim was filled by a recognisable narrative pattern, describe the PATTERN structurally. Examples of acceptable outputs: "the absence of peer-reviewed primary research was filled by synchronised fake-expert commentary", "a contemporaneous-news vacuum was filled by repeated numeric coincidences stacking toward a single conclusion". UNACCEPTABLE outputs: naming specific publishers, individuals, organisations, think-tanks, or campaigns; conflating authentic grassroots voice with engineered campaigns (the Charter Commitment 3 guard: legitimate dissent must never be characterised as astroturf absent documented coordinated inauthentic behaviour). If no vacuum-filling pattern is detected, leave the field as an empty string. This is a strict scope constraint; violations are treated as a regression.
